...
interface Props {
  ...
  onChangeAccessType: (order: AccessType) => void
  isSide?: boolean
}

export default function PublishSection({ onChangeAccessType, accessType, isSide }: Props) {
  const { close, onClick, onTransitionEnd, ref } = useDataDrivenAnimation<HTMLDivElement>()
  ...
  return (
    ...
    >
      <DropDown.Trigger targetRef={accessTypeRef} variant="icon" onClick={onClick}>
      ...
      >
        <DropDown.Button
          variant="list"
          onClick={() => onChangeAccessType(AccessType.Public)}
          className="gap-2"
        >
          공개
          {accessType === AccessType.Public && (
        ...
        <DropDown.Button
          variant="list"
          onClick={() => onChangeAccessType(AccessType.Private)}
          className="gap-2"
        >
          비공개
