...
export default function AuthHistory({ userId }: Props) {
  ...
  const { data: postLength } = useSuspenseQuery(postCountQuery.countAllMyPost(supabase, userId))
  ...
  return (
    <Container className="animate-fade-in">
      <XStack className="sm:gap-8">
        <HistoryBlock title="시작한지" content={getSignUpDays(user?.created_at)} unit="일 째" />
